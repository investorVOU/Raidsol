  ...
  return (
    <div className="h-full flex flex-col p-6 animate-in slide-in-from-right duration-300 overflow-y-auto scrollbar-hide pb-24">
      ...
      <div className="bg-black border-2 border-purple-500/40 tech-border p-8 relative overflow-hidden mb-10">
        <div className="absolute -top-4 -right-4 opacity-5 mono text-8xl font-black italic">
            [RAID]
        </div>
        
        <div className="flex items-center gap-3 mb-4">
            <span className="text-xs font-black uppercase tracking-[0.2em] italic border border-purple-400/30 px-3 py-1 text-purple-400 bg-purple-400/10">Mega Raid Series #14</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-8">
            <div className="flex items-center gap-2">
                <span className="mono text-xs font-black text-white/40 uppercase tracking-widest italic">[TIME] 14:02:44</span>
            </div>
            <div className="flex items-center gap-2">
                <span className="mono text-xs font-black text-purple-400 uppercase tracking-widest italic">[FEE] {ENTRY_FEES[Mode.TOURNAMENT]} SOL</span>
            </div>
        </div>
      </div>
      ...
      <div className="flex-1">
        ...
        <div className="space-y-4">
            {leaderboard.map((entry) => (
                <div 
                    key={entry.rank}
                    className="group bg-black border-2 border-white/5 p-6 tech-border flex items-center justify-between hover:border-purple-500/30 transition-all cursor-pointer"
                >
                    <div className="flex items-center gap-6">
                        <div className={`w-12 flex justify-center mono font-black italic text-2xl ${
                            entry.rank === 1 ? 'text-yellow-500' : entry.rank === 2 ? 'text-white/60' : entry.rank === 3 ? 'text-orange-400' : 'text-white/20'
                        }`}>
                            #{entry.rank}
                        </div>
                        <div>
                            <p className="text-lg font-black tracking-tight uppercase italic">{entry.name}</p>
                            <p className="mono text-xs text-white/20 uppercase tracking-widest">{entry.score.toLocaleString()} PTS</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <p className="mono text-lg font-black text-purple-400 italic">{entry.prize}</p>
                        <p className="text-[10px] text-white/10 font-black uppercase tracking-[0.3em]">BOUNTY</p>
                    </div>
                </div>
            ))}
        </div>
