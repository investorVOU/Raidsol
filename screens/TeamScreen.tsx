  ...
  return (
    <div className="h-full flex flex-col p-6 animate-in slide-in-from-right duration-300 overflow-y-auto scrollbar-hide pb-24">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-black italic uppercase tracking-tighter">Squad <span className="text-cyan-400">Gamma-9</span></h2>
          <div className="flex items-center gap-1.5 text-white/40 mt-1">
            <span className="text-xs font-black uppercase tracking-wider italic">[SQUAD_ACTIVE] 4/4 Members Ready</span>
          </div>
        </div>
      ...
      <div className="bg-black border-2 border-white/5 p-8 mb-10 relative overflow-hidden tech-border">
        <div className="absolute top-0 right-0 p-5 opacity-5 mono text-3xl font-black italic">
            [SYNC]
        </div>
        ...
        <div className="flex justify-between mt-4">
          ...
          <div className="flex items-center gap-2 text-green-400">
            <div className="w-2 h-2 bg-green-400 rounded-none animate-pulse" />
      ...
      <div className="flex-1 space-y-4">
        {players.map(p => (
          <div key={p.id} className={`p-5 border-2 tech-border flex items-center justify-between ${
            p.name === 'You' ? 'bg-cyan-950/20 border-cyan-500/30' : 'bg-black border-white/5'
          }`}>
            <div className="flex items-center gap-5">
              <div className={`w-10 h-10 flex items-center justify-center font-black text-sm border ${
                p.status === 'EXITED' ? 'bg-green-500/20 text-green-500 border-green-500/30' : 'bg-white/10 text-white border-white/10'
              }`}>
                {p.name[0]}
              </div>
              <div>
                <p className="text-base font-black tracking-tight uppercase italic">{p.name} {p.name === 'You' && '(Commander)'}</p>
            ...
            <div className="text-right">
              <p className="mono text-base font-bold">{p.solEarned.toFixed(2)} SOL</p>
              <p className="text-[10px] text-white/40 font-black uppercase tracking-widest italic">YIELD</p>
            </div>
          </div>
        ))}
      </div>
