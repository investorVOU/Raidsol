  ...
  if (activeRoom) {
    return (
      <div className="h-full flex flex-col animate-in fade-in duration-300 relative bg-[var(--modal-bg)]">
        ...
        {/* Join notification */}
        {joinNotification && (
          <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[300] w-max max-w-xs bg-[#FF2929]/12 border border-[#FF2929] px-5 py-3 text-xs font-bold text-white animate-in slide-in-from-top-4 duration-300 shadow-[0_0_20px_rgba(255,41,41,0.3)]">
            {joinNotification}
          </div>
        )}
        ...
        <div className="shrink-0 flex justify-between items-center px-4 sm:px-6 py-3 border-b border-white/5 bg-[var(--modal-bg)] z-10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-white/40">· {players.length}/{activeRoom.maxPlayers} online</span>
          </div>
          <button onClick={onLeaveRoom} className="text-[10px] font-bold text-red-500/70 hover:text-red-400 border border-red-500/20 hover:border-red-500/50 px-3 py-1.5 transition-all">
            Leave room
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-4">
            ...
            {/* ── CODE + QR ── */}
            <div className="bg-[#09090b] border border-[#FF2929]/40 tech-border relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6 p-4 sm:p-6">
                ...
                <div className="flex-1 w-full text-center sm:text-left">
                  <p className="text-[10px] font-bold text-white/50 mb-2">Access code</p>
                  <button
                    onClick={copyCode}
                    className="group inline-flex items-center gap-3 px-5 py-3 bg-white/5 border border-white/10 hover:border-[#FF2929]/50 hover:bg-[#FF2929]/5 transition-all mb-2"
                  >
            ...
            <div className="bg-[#09090b] border border-white/8 tech-border overflow-hidden">
              <div className="px-4 py-3 border-b border-white/5 flex justify-between items-center">
                <span className="text-[10px] font-bold text-white/60">Players</span>
                <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: '#FF2929' }}>
              ...
              <div className="divide-y divide-white/5">
                {players.map((player, i) => (
                  <div key={player.id} className="flex items-center gap-3 px-4 py-3 animate-in slide-in-from-left duration-300" style={{ animationDelay: `${i * 80}ms` }}>
                    <div className="w-6 h-6 border border-white/25 bg-white/8 flex items-center justify-center shrink-0">
                      <span className="text-[9px] font-black text-white/60">{String(i + 1).padStart(2, '0')}</span>
                    </div>
                    <span className="flex-1 text-sm font-bold text-white">
                      {player.name} {player.id === walletAddress && <span className="text-[9px] text-white/40">(you)</span>}
                    </span>
                    <span className="text-[9px] font-bold text-white/70 bg-white/8 border border-white/15 px-2 py-0.5">Ready</span>
                  </div>
                ))}
                {[...Array(activeRoom.maxPlayers - players.length)].map((_, i) => (
                  <div key={i} className="flex items-center gap-3 px-4 py-3 opacity-30">
                    ...
                    <span className="flex-1 text-sm font-bold text-white/40">Waiting for player...</span>
            ...
            <div className="bg-[#09090b] border border-white/8 tech-border overflow-hidden">
                ...
                {chatMessages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] px-2.5 py-1.5 ${isMe ? 'bg-[#FF2929]/12 border border-[#FF2929]/25' : 'bg-white/5 border border-white/8'}`}>
                        {!isMe && <p className="text-[8px] font-bold text-white/50 mb-0.5">{msg.username}</p>}
                        <p className={`text-[11px] font-medium ${isMe ? 'text-white/80' : 'text-white/70'}`}>{msg.content}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="flex gap-1.5 p-2 border-t border-white/5">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#FF2929]/15 border border-[#FF2929]/30 text-[#FF2929]/80 text-xs font-bold hover:bg-[#FF2929]/25 transition-colors disabled:opacity-20 disabled:cursor-not-allowed"
                >
                  Send
                </button>
              </div>
          ...
          {isHost ? (
            <button
              ...
              className={`w-full py-5 sm:py-6 tech-border font-bold uppercase tracking-tight text-xl sm:text-2xl transition-all
                ${canStart
                  ? 'bg-[#FF2929] text-white shadow-[0_0_40px_rgba(255,41,41,0.35)] hover:bg-[#CC0000] active:scale-[0.99]'
        ...
        <div className="fixed inset-0 z-[200] bg-[var(--modal-bg)] flex flex-col">
          ...
          <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-[#FF2929]/30 bg-[var(--modal-bg)]">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 border border-white/20 hover:border-white/50 text-white/70 hover:text-white transition-all text-xs font-bold"
            >
              <X size={14} /> Cancel
            </button>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center p-6 min-h-0">
            <div className="relative w-full max-w-xs">
              ...
              <div className="absolute inset-0 pointer-events-none">
                ...
                <div className="absolute top-2 right-2 w-6 h-6 border-t-4 border-r-4 border-[#FF2929]" />
                <div className="absolute bottom-2 left-2 w-6 h-6 border-b-4 border-l-4 border-[#FF2929]" />
                <div className="absolute bottom-2 right-2 w-6 h-6 border-b-4 border-r-4 border-[#FF2929]" />
              </div>
            </div>
          </div>
      ...
      <div className="shrink-0 flex items-center gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b border-white/5 bg-[var(--modal-bg)]">
        ...
        <div>
          <h2 className="text-xl sm:text-2xl font-black tracking-tight text-white leading-none">
            PvP <span className="text-[#FF2929]">Arena</span>
          </h2>
        ...
        <div className="max-w-lg mx-auto p-4 sm:p-6 flex flex-col min-h-full">
          ...
          {view === 'MENU' && (
            <div className="flex flex-col gap-4 flex-1 justify-center">
              ...
              {/* Banner */}
              <div className="border border-[#FF2929]/25 bg-[#FF2929]/4 p-4 sm:p-5 tech-border text-center">
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] font-bold text-white/50">Enter code or scan QR</p>
                </div>
                <span className="text-xl font-black text-white/40 group-hover:text-[#FF2929] transition-colors">→</span>
              </button>
          ...
          {view === 'CREATE' && (
            <div className="space-y-5 py-2">
              ...
              {/* Currency selector */}
              <div>
                <p className="text-[10px] font-bold text-white/70 mb-2">01 · Stake currency</p>
                <div className="grid grid-cols-3 gap-2">
                  {([Currency.SOL, Currency.USDC, Currency.SKR] as Currency[]).map(c => {
                    const active = stakeCurrency === c;
                    const col = c === Currency.SOL ? 'border-[#FF2929] text-[#FF2929] bg-[#FF2929]/10'
              ...
              {/* Stake amount */}
              <div>
                <p className="text-[10px] font-bold text-white/70 mb-2">02 · Entry stake</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-[10px] font-bold text-white/40 shrink-0">{CURRENCY_LABELS[stakeCurrency]}</span>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <p className="text-[10px] font-bold text-white/70">03 · Max players</p>
                  <span className="text-sm font-black text-[#FF2929] mono">{maxPlayers}</span>
                </div>
                <input
                ...
                />
                <div className="flex justify-between text-[9px] font-bold text-white/40 mt-1">
                  <span>2 min</span>
                  <span>10 max</span>
                </div>
              </div>
              ...
              {/* Pool preview */}
              <div className="p-4 sm:p-5 bg-[#09090b] border border-white/12 tech-border">
                <div className="flex justify-between items-center mb-3 pb-3 border-b border-white/5">
                  <span className="text-[10px] font-bold text-white/50">Stake per player</span>
                  <span className="font-black mono text-white">{stakeAmount} {CURRENCY_LABELS[stakeCurrency]}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-white/50">Max total pot</span>
              ...
              <button
                ...
                disabled={isCreating}
                className="w-full py-5 sm:py-6 bg-[#FF2929] text-white tech-border font-bold uppercase tracking-tight text-xl sm:text-2xl shadow-[0_0_25px_rgba(255,41,41,0.3)] hover:bg-[#CC0000] active:scale-[0.99] transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              >
          ...
          {view === 'JOIN' && (
            <div className="space-y-4 py-2">
              ...
              <div>
                <p className="text-[10px] font-bold text-white/70 mb-2">Room code</p>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="shrink-0 bg-white/5 border-2 border-white/15 tech-border px-3 sm:px-4 hover:bg-[#FF2929]/10 hover:border-[#FF2929]/50 text-white/60 hover:text-[#FF2929] transition-all"
                    title="Scan QR Code"
                  >
                    <Scan size={20} />
                  </button>
              ...
              {previewError && (
                <div className="flex items-center gap-2 p-3 border border-red-500/30 bg-red-500/5">
                  ...
                  <p className="text-red-400 text-[11px] font-bold">{previewError}</p>
                </div>
              )}

              {joinPreview && (
                <div className="bg-[#09090b] border border-[#FF2929]/35 tech-border overflow-hidden">
                  <div className="px-4 py-2 bg-[#FF2929]/8 border-b border-[#FF2929]/18">
              ...
              {!joinPreview ? (
                <button
                  ...
                  className="w-full py-5 bg-white text-black tech-border font-bold uppercase tracking-tight text-xl hover:bg-[#FF2929] hover:text-white transition-all active:scale-[0.99] disabled:opacity-40 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  disabled={isJoining}
                  style={{ background: "linear-gradient(135deg, #FF2929 0%, #CC0000 100%)", boxShadow: "0 0 20px rgba(255,41,41,0.30)" }} className="w-full py-5 text-white tech-border font-bold uppercase tracking-tight text-xl hover:brightness-110 active:scale-[0.99] transition-all disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="w-full py-5 bg-[#FF2929] text-white tech-border font-bold uppercase tracking-tight text-xl shadow-[0_0_25px_rgba(255,41,41,0.3)] hover:bg-[#CC0000] active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
